...
| 指标                             | 含义                                           | 方向     | 当前公开结果 |
| -------------------------------- | ---------------------------------------------- | -------- | ------------ |
| Faithfulness                     | 回答中的陈述能否由检索上下文支持，反映幻觉程度 | 越高越好 | 0.830141     |
| Answer Relevancy                 | 回答是否直接回应用户问题，是否存在跑题或冗余   | 越高越好 | 0.81588      |
| Context Precision                | 排在前面的检索上下文中，真正相关内容所占程度   | 越高越好 | 0.977011     |
| Context Entity Recall            | 参考答案中的关键实体有多少出现在检索上下文中   | 越高越好 | 0.129885     |
| Noise Sensitivity (`irrelevant`) | 回答受到无关上下文误导的程度                   | 越低越好 | 0.022857     |
| Context Recall                   | 参考答案所需的信息有多少被检索上下文覆盖       | 越高越好 | 1.0          |
